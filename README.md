...
| `VOLUME_Z_THRESHOLD` | 3.0 | **出来高異常のZ-score閾値**<br>・標準偏差の倍数<br>・`正の値` = 上昇異常のみ検知<br>・`負の値` = 下落異常のみ検知<br>・`0` = すべての変化を検知 | `1.5` = 出来高1.5σ以上の増加<br>`-1.5` = 出来高1.5σ以上の減少<br>`3` = 出来高3σ以上の増加 |
| `PRICE_Z_THRESHOLD` | 3.0 | **価格異常のZ-score閾値**<br>・標準偏差の倍数<br>・`正の値` = 価格上昇異常のみ検知<br>・`負の値` = 価格下落異常のみ検知<br>・`0` = すべての変化を検知 | `1.5` = 価格1.5σ以上の上昇<br>`-1.5` = 価格1.5σ以上の下落<br>`2` = 価格2σ以上の上昇 |
...
#### 例4: 価格急騰狙いの売り設定
```env
PRICE_MULTIPLIERS=1.5,2.0,2.5
ORDER_AMOUNTS_USDC=500,500,500
DETECTION_MODE=price_only
PRICE_Z_THRESHOLD=2.0
```
- 価格上昇異常のみ検知（2σ以上）
